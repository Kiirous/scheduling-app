...
class _SchedulingsListAreaState extends State<SchedulingsListArea> with AutomaticKeepAliveClientMixin {
  late final SchedulingsCubit _cubit = SchedulingsCubit(status: widget.status);

  @override
  void initState() {
    super.initState();
    _cubit.loadSchedulings();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider.value(
      value: _cubit,
      child: BlocBuilder<SchedulingsCubit, SchedulingsState>(
        builder: (context, state) {
          ...
          return Padding(
            ...
            child: ListView.builder(
              ...
              itemCount: state.schedulings!.length + (state.finishedLoading ? 0 : 1),
              ...
              itemBuilder: (_, i) {
                if (i < state.schedulings!.length) {
                  ...
                  return SizedBox(
                    height: 120 + 18,
                    child: HomeNextScheduleItem(scheduling: scheduling, shadowOffset: const Offset(3, 6)),
                  );
                } else {
                  _cubit.loadSchedulings();
                  return const Center(child: AppLoadingIndicator());
